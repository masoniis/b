use crate::{
    ...
    render_world::{
        ...
        passes::{
            core::{create_render_pipeline_from_def, CreatedPipeline, PipelineDefinition},
            world::main_passes::shared_resources::{
                main_depth_texture::MAIN_DEPTH_FORMAT, CentralCameraViewBindGroupLayout,
                EnvironmentBindGroupLayout,
            },
        },
        types::vertex::WireframeVertex,
    },
};
...
pub fn setup_bb_pipeline_and_buffers(
    ...
    view_layout: Res<CentralCameraViewBindGroupLayout>, // shared @group(0) camera layout
    ...
    let wireframe_pipeline_def = PipelineDefinition {
        ...
        material_path: "assets/shaders/world/main_passes/wireframe/main.material.ron",
        vs_shader_source: wgpu::ShaderSource::Wgsl(include_wesl!("wireframe_vert").into()),
        fs_shader_source: Some(wgpu::ShaderSource::Wgsl(
            include_wesl!("wireframe_frag").into(),
        )),
        vertex_buffers: &[WireframeVertex::desc()],
