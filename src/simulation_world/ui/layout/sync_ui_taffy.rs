use crate::{
    ...
    simulation_world::{
        ...
        ui::{
            components::{self as simulation, CalculatedLayout},
...
pub fn compute_and_apply_layout_system(world: &mut World) {
    ...
    let mut root_query = world.query_filtered::<Entity, With<simulation::UiRoot>>();
...
fn apply_layouts_recursively(
    ...
    if world.get::<simulation::UiText>(entity).is_some() {
        debug!(
            target: "ui_layout",
    ...
    } else if world.get::<simulation::UiRoot>(entity).is_some() {
        debug!(
            target: "ui_layout",
