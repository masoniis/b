// src/assets/shaders/scene/simple.wgsl
...
struct VertexInput {
  @location(0) position : vec3<f32>,
                          @location(1) color : vec3<f32>,
                                               @location(2) model_row_0
      : vec4<f32>,
        @location(3) model_row_1 : vec4<f32>,
                                   @location(4) model_row_2
      : vec4<f32>,
...
@vertex fn vs_main(model_in : VertexInput) -> VertexOutput {
  ...
  out.clip_position =
      camera.view_proj * model_matrix * vec4<f32>(model_in.position, 1.0);
